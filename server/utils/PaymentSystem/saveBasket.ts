...
export const SavePayment = async (paymentRequest: IPaymentRequest, user_id: string, order_id: string, payment_type: string, paymentId: string) => {
	const billingAddress: AddressField = (await Address.findOne({
		user_id: user_id,
		zip_code: paymentRequest.billingAddress.zipCode,
		address: paymentRequest.billingAddress.address
	})) as AddressField;
	const shippingAddress: AddressField = (await Address.findOne({
		user_id: user_id,
